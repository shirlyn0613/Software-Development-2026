...
python exp3_reservoir_optimization\validation.py
python -m unittest discover exp3_reservoir_optimization
...
- The root `requirements.txt` gives a shared dependency list across the four experiments.
- Each experiment has it's own dependency in it's own folder's ``requirements.txt``
